...
- `location /api/ { proxy_pass http://${API_UPSTREAM}/; proxy_buffering off; proxy_cache off; }` — the trailing `/` strips the `/api` prefix (backend routes are root-mounted, no global prefix); `${API_UPSTREAM}` is `127.0.0.1:3000` on ECS (shared task loopback) and `api:3000` under docker-compose. SSE tokens flush immediately (this matters for the streaming SOAP UX).
